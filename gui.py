...
class Ui_Spam_detector(object):
    ...
    def button_click(self):
        ...
        X_train, X_test, y_train, y_test = train_test_split(data['URL'], data['Target'], test_size=0.2, random_state=42)

        # Create a vectorizer to convert the URLs into feature vectors
        vectorizer = TfidfVectorizer()

        # Fit the vectorizer to the training data and transform the URLs into feature vectors
        X_train_vect = vectorizer.fit_transform(X_train)

        # Load the saved classifier from the file
        classifier = joblib.load('./spam_classifier.joblib')

        # Define a new URL to classify
        new_url = text

        # Transform the new URL into a feature vector using the same vectorizer as before
        new_url_vect = vectorizer.transform([new_url])

        # Use the loaded classifier to predict whether the new URL is spam or not spam
        prediction = classifier.predict(new_url_vect)
        # Print the predicted label
        if prediction[0] == "yes":
            prediction_text = "The URL is spam."
        else:
            prediction_text = "The URL is not spam."

        self.output_text.append(prediction_text)
